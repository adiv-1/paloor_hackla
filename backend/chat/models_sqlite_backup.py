"""
Chat persistence layer — PostgreSQL via psycopg2.
...
def list_conversations(user_id: str, conv_type: str = None, include_archived: bool = False) -> List[dict]:
    with pg_cursor() as cur:
        sql = """
            SELECT c.*, cm.position FROM conversations c
        ...
        for row in rows:
            ...
            d["metadata"] = d.get("metadata") or {}
            cur.execute(
                "SELECT content, sender_name, is_ai_generated FROM messages WHERE conversation_id = %s ORDER BY created_at DESC LIMIT 1",
                (d["id"],)
            )
...
def reorder_conversations(user_id: str, conversation_ids: List[str]) -> bool:
    """Set position for a user's conversations based on the provided order."""
    with pg_cursor() as cur:
...
def join_group(conv_id: str, user_id: str) -> bool:
    with pg_cursor() as cur:
        ...
        try:
            cur.execute(
                "INSERT INTO conversation_members (conversation_id, user_id, role, joined_at) VALUES (%s, %s, 'member', %s)",
...
def list_messages(conversation_id: str, limit: int = 50, before: float = None) -> List[dict]:
    with pg_cursor() as cur:
        ...
        for row in reversed(rows):  # Return in chronological order
            d = dict(row)
            d["metadata"] = d.get("metadata") or {}
            cur.execute("SELECT * FROM attachments WHERE message_id = %s", (d["id"],))
            atts = cur.fetchall()
            d["attachments"] = [dict(a) for a in atts]
            results.append(d)
        return results
...
def upsert_user_context(user_id: str, context: dict, summary: str) -> dict:
    ...
    return {"user_id": user_id, "context": context, "summary": summary, "updated_at": now}
...
def list_folders(user_id: str) -> List[dict]:
    with pg_cursor() as cur:
        ...
        for row in rows:
            ...
            items = cur.fetchall()
            d["conversation_ids"] = [i["conversation_id"] for i in items]
            results.append(d)
        return results
...
def add_to_folder(folder_id: str, conversation_id: str) -> bool:
    with pg_cursor() as cur:
        try:
            cur.execute(
                "INSERT INTO chat_folder_items (folder_id, conversation_id) VALUES (%s, %s)",
